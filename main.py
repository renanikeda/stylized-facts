...
## Asset indexes
def plot_asset_hist(df_assets, plot_later = True):
    global fig
    global axs
    global colors_asset
    fig[0], axs[0] = plt.subplots(2)
    for (index, asset) in enumerate(df_assets):
        axs[0][index].set_title(asset)
        # axs[1].set_title("S&P500")

        df_assets[asset].plot(color=color_line[index][1], linestyle=color_line[index][0], ax=axs[0][index], legend = True)
    ...
    plt.tight_layout()
    if not plot_later: plt.show()

## Autocorrelation Returns
def plot_autocorrelation_returns(return_assets, plot_later = True):
    global fig
    global axs
    global colors_asset
    for (index_asset, asset) in enumerate(return_assets):
        total_index = get_none_index(fig)
        fig[total_index] = plt.figure(figsize=(10, 5))
        for (index, period) in enumerate(return_assets[asset]):
            lags = 20
            alpha = 0.05
            ax = fig[total_index].add_subplot(int(f'13{index + 1}'))

            sm.graphics.tsa.plot_acf(return_assets[asset][period], lags=lags, ax=ax, alpha=alpha, color=colors_asset[asset])
            plt.xlabel('lags')
...
def calc_kurtosis(df_assets, return_assets):
    for asset in df_assets:
        for period in return_assets[asset]:
            print(f'Curtose Retorno {period.title()} {asset}', round(kurtosis(return_assets[asset][period], axis=0, bias=True, fisher=False), 2))
        print('\n')


## Calculando a Assimetria
def calc_skewness(df_assets, return_assets):
    for asset in df_assets:
        for period in return_assets[asset]:
            print(f'Assimetria Retorno {period.title()} {asset}', round(skew(return_assets[asset][period], axis=0, bias=True), 2))
        print('\r')


## Agregação Gaussiana
def plot_gaussian_aggregation(return_assets, plot_later = True):
    global fig
    global axs
    global colors_asset
    for (index_asset, asset) in enumerate(return_assets):
        ...
        for (index_period, period) in enumerate(return_assets[asset]):
            ax = fig[total_index].add_subplot(int(f'23{index_period + 1}'))
            sns.histplot(return_assets[asset][period], bins=50, color=colors_asset[asset], stat='density', ax=ax)
            xmin, xmax = plt.xlim()
            x = np.linspace(xmin, xmax, 100)
            p = norm.pdf(x, np.mean(return_assets[asset][period]), np.std(return_assets[asset][period]))
            ...
            qq = qqplot(return_assets[asset][period], line='s', ax=ax, markerfacecolor=colors_asset[asset], fit=True)
...
## Intermitência
def plot_intermittency(return_assets, plot_later = True):
    global fig
    global axs
    global colors_asset
    for asset in return_assets:
        ...
        for (index_period, period) in enumerate(return_assets[asset]):
            ...
            return_assets[asset][period].plot(figsize=(10, 7), color=colors_asset[asset], linestyle='-', ax=ax)
            plt.legend(fontsize="10")
            plt.xticks(rotation=45, ha='right')
            plt.xlim([return_assets[asset][period].index[0], return_assets[asset][period].index[-1]]);
            plt.ylabel(f'{asset} retorno {period}')
            ax.legend(['Fechamento'])
        plt.tight_layout()
    if not plot_later: plt.show()

def plot_volatility(return_assets, plot_later = True):
    global fig
    global axs
    global colors_asset
    for asset in returns:
        ...
        for (index_period, period) in enumerate(return_assets[asset]):
            ax = fig[total_index].add_subplot(int(f'31{index_period + 1}'))
            std_returns = standard_deviation(return_assets[asset][period], window = 15)
...
def plot_volatility_clustering(return_assets, plot_later = True):
    global fig
    global axs
    global colors_asset
    total_index = get_none_index(fig)
    ...
    for (index_asset, asset) in enumerate(return_assets):
        for (index_period, period) in enumerate(return_assets[asset]):
            ...
            sm.graphics.tsa.plot_acf(return_assets[asset][period]**2, lags=40, ax=ax, alpha=0.05, color=colors_asset[asset])
            plt.xlabel('lags')
...
def plot_conditional_heavy_tails(return_assets, plot_later = True):
    global fig
    global axs
    global colors_asset
    ...
    for (index_asset, asset) in enumerate(return_assets):
        ...
        for (index_period, period) in enumerate(return_assets[asset]):
            ax = fig[total_index].add_subplot(int(f'23{index_period + 1}'))
            garch = arch_model(100*return_assets[asset][period], vol='GARCH', p=1, q=1, dist='gaussian')
...
def plot_absolute_returns_decay(return_assets, plot_later = True):
    global fig
    global axs
    global colors_asset
    ...
    for (index_asset, asset) in enumerate(return_assets):
        for (index_period, period) in enumerate(return_assets[asset]):
            ax = fig[total_index].add_subplot(int(f'23{index_period+1 + (index_asset*3)}'))
            sm.graphics.tsa.plot_acf(abs(return_assets[asset][period]), lags=lags, ax=ax, alpha=0.05, color=colors_asset[asset])
            popt = fit_power_law(abs(return_assets[asset][period]), lags = lags)
...
## Efeito Alavancagem
def plot_leverage_effect(return_assets, plot_later = True):
    global fig
    global axs
    global colors_asset
    ...
    for (index_asset, asset) in enumerate(return_assets):
        ...
        std_returns = standard_deviation(return_assets[asset][period], window = 10)
        std_returns.plot(figsize=(10, 7), color=colors_asset[asset], linestyle='-', ax=ax)
        plt.legend(fontsize="10")
        plt.xticks(rotation=45, ha='right')
        plt.xlim([return_assets[asset][period].index[0], return_assets[asset][period].index[-1]]);
    ...
    for (index_asset, asset) in enumerate(return_assets):
        ...
        for (index_period, period) in enumerate(return_assets[asset]):
            ax = fig[total_index].add_subplot(int(f'31{index_period + 1}'))
            std_returns = standard_deviation(return_assets[asset][period], window = window, clean=False)
            ...
            sns.regplot(x=std_returns, y=return_assets[asset][period].shift(1)[1:], color=colors_asset[asset], ax=ax)
            # data = pd.concat([std_returns.diff().dropna(), return_assets[asset][period].shift(1)], axis=1)
            # data = data.set_axis(['x', 'y'], axis=1).dropna()
            # g = sns.lmplot(x='x', y='y', data=data)
            # g.map_dataframe(annotate)
            plt.ylabel(f'{asset} retorno {period}')
            plt.xlabel(f'{asset} volatilidade {period}')
            print(f"Correlação entre a Volatilidade e o retorno do {asset}: ", std_returns.diff().dropna().corr(return_assets[asset][period].shift(1)[1:]))
        plt.tight_layout()
    print('\n')

#Correlação entre volume e volatilidade
def plot_volume_volatility_correlation(return_assets, plot_later = True):
    global fig
    global axs
    global colors_asset
    for (index_asset, asset) in enumerate(return_assets):
...
# plot_asset_hist(df_total)
# plot_autocorrelation_returns(returns)
# calc_kurtosis(df_total, returns)
# calc_skewness(df_total, returns)
# plot_gaussian_aggregation(returns)
# plot_intermittency(returns)
# plot_volatility(returns)
# plot_volatility_clustering(returns)
# plot_conditional_heavy_tails(returns)
# plot_absolute_returns_decay(returns)
# plot_leverage_effect(returns)
plot_volume_volatility_correlation(returns)
